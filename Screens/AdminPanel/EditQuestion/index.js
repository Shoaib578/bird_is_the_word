...
export default class EditQuestion extends React.Component {
    ...
    getCurrentValues = ()=>{
        firestore().collection("questions").doc(this.props.route.params.id).get()
        .then(res=>{
           console.log(res._data.question_type)
           if(res._data.question_type){
               this.setState({
                   question_type:res._data.question_type,
                   option_1:res._data.option_1,
                   option_2:res._data.option_2,
                   option_3:res._data.option_3,
                   question:res._data.question,
                   is_option_question:true
               })
           }else{
            this.setState({
                question_type:res._data.question_type,
    ...
    EditOptionQuestion = ()=>{
        ...
        firestore().collection('questions').doc(this.props.route.params.id)
        .update({
    ...
    EditNormalQuestion = ()=>{
        ...
        .then(res=>{
            ...
            Alert.alert("Question Update Successfully")
        })
        .catch(err=>{
            this.setState({isLoading:false})
            Alert.alert("Something Went Wrong")
